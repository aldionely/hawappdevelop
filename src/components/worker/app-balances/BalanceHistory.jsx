...
export const BalanceHistory = ({ transactions, products }) => {
  const { currentUserAppBalanceLogs } = useData();
  const [filter, setFilter] = useState('ALL');

  const manualLogs = useMemo(() => (currentUserAppBalanceLogs || []).map(log => ({ ...log, source: 'manual' })), [currentUserAppBalanceLogs]);

  const combinedLogs = useMemo(() => {
    const logsFromTransactions = (transactions || []).map(tx => convertTxToBalanceLog(tx, products)).filter(Boolean);
    const allLogs = [...manualLogs, ...logsFromTransactions];
    return allLogs.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  }, [manualLogs, transactions, products]);
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-between items-center">
        <h3 className="font-semibold text-sm">Riwayat Saldo Aplikasi</h3>
      ...
      <div className="max-h-[300px] overflow-y-auto pr-2 space-y-2 text-xs">
        {filteredLogs.length === 0 && <p className="text-center text-gray-500 pt-4">Tidak ada riwayat untuk ditampilkan.</p>}
